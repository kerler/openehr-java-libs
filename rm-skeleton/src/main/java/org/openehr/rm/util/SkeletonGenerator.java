...
public class SkeletonGenerator {
	 ...
	 * @throws Exception if there's difficulty initializing the default settings
	 * @deprecated use the instance constructors, preferably without using default settings
	 */
	public static SkeletonGenerator getInstance() throws Exception {
		return new SkeletonGenerator();
	}
	
	public SkeletonGenerator() throws Exception {
		this(
	...
	public SkeletonGenerator(TerminologyService ts, MeasurementService ms, TermMap tm,
			RMObjectBuilder b) {
	...
	private static RMObjectBuilder initializeBuilder(TerminologyService ts, MeasurementService ms) {
	...
	public void loadTermMapFromClasspath(String filename) throws Exception {
		...
		if (input == null) {
			throw new IllegalArgumentException("Can not open filename=" + filename);
		}
		termMap.clear();
		termMap.addAll(input);
	}
	...
	//
	// introspection and object walking
	//
	protected Object createComplexObject(CComplexObject ccobj,
		...
		archetype = processNodeId(ccobj, archetype, archetypeMap, valueMap);

		createAttributes(ccobj, archetype, archetypeMap, extraValues, strategy, valueMap, rmTypeName);

		// special case for DV_MULTIMEDIA
		if ("DV_MULTIMEDIA".equals(rmTypeName)) {
			return createDvMultimedia();
		}

		// special case for INTERNAL_EVENT
		if ("INTERVAL_EVENT".equals(rmTypeName)) {
			return null;
		}

		addDefaultValuesForRequiredAttributes(ccobj, archetype, strategy, valueMap, rmTypeName);
	...
	private Archetype processNodeId(CComplexObject ccobj, Archetype archetype, Map<String, Archetype> archetypeMap,
	...
	protected void createAttributes(CComplexObject ccobj, Archetype archetype, Map<String, Archetype> archetypeMap,
			Map<String, Object> extraValues, GenerationStrategy strategy, Map<String, Object> valueMap,
			String rmTypeName) throws Exception {
	...
	private void addDefaultValuesForRequiredAttributes(CComplexObject ccobj, Archetype archetype,
			GenerationStrategy strategy, Map<String, Object> valueMap, String rmTypeName) throws Exception {
	...
	private void addEntryValues(Map<String, Object> valueMap,
		...
		valueMap.put("subject", createSubject());
	...
	private Object createDvMultimedia() throws Exception {
		CodePhrase charset = new CodePhrase("IANA_character-sets", "UTF-8");
	...
	protected PartySelf createSubject() throws Exception {
